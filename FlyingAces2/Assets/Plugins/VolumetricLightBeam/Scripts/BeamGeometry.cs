...
#pragma warning restore 0162
        ...
        {
    #if UNITY_EDITOR
            if(cam != null && !IsEditorCamera(cam)) // using OnBeginCameraRendering cb for editor camera too will make some glitchy flickering
    #endif
            {
        ...
        void OnWillRenderObject()
        {
            ...
            {
            #if UNITY_EDITOR
                if(!IsEditorCamera(Camera.current)) { return; } // in editor SRP, this cb is only called with a valid camera for editor camera 
            #else
                return; // in standalone SRP builds, do not use it
            #endif
            }
    #endif
