        ...
        void DrawAdditionalFeatures()
        {
...
#endif
            bool showButtonOcclusion    = m_Targets.HasAtLeastOneTargetWith((VolumetricLightBeam beam) => { return beam.GetComponent<DynamicOcclusion>() == null; });
            ...
            if (showButtonDust || showButtonOcclusion || showButtonTriggerZone)
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    {
                        AddComponentToTargets<DynamicOcclusion>();
                    }
